...
class PiCreature(SVGMobject):
    ...
    def __init__(self,
        ...
        super().__init__(file_name=self.get_svg_file_path(mode),
                         color=None,
    ...
    def shrug(self):
        ...
        top_mouth_point, bottom_mouth_point = [points[np.argmax(points[:, 1])], points[np.argmin(points[:, 1])]]
    ...
    def replace_bubble(self, content, mode="pondering", look_at=None, **kwargs) -> Animation | _AnimationBuilder:
        ...
        return AnimationGroup(ReplacementTransform(old_bubble, new_bubble),
                              FadeTransform(old_bubble.content, new_bubble.content), self.change(mode, look_at))
    ...
    def debubble(self, mode="plain", look_at=None, **kwargs):
        ...
        result = RemovePiCreatureBubble(self, target_mode=mode, look_at=look_at, **kwargs)
        self.bubble = None
        return result
...
class Eyes(VGroup):
    ...
    def create_eyes(self, mode: str, height: float, bottom: Vect3, look_at: Vect3 | Mobject | None = None):
        self.mode = mode
